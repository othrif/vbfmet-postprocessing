...
void makeJetFakePh(std::string treeNmae="data") {
  ...
  for (Long64_t i=0;i<nentries; i++) {
    ...
    if (n_ph==0 && baseph_pt && baseph_pt->size()==1){// extrapolating from a base photon with selection X
      ...
      bool passBasePhotonAntiIDSel=true;
      // apply the selections that we want. maybe label them?
      // note that these base photons are not in the MET, so we may need to correct for that. it's a touch question because the isolation wouldn't be included...
      //baseph_pt
      n_ph=1;
      ph_pt->push_back(baseph_pt->at(0));
      ph_eta->push_back(baseph_eta->at(0));
      ph_phi->push_back(baseph_phi->at(0));
      if(ph_isEM && baseph_isEM)                 ph_isEM->push_back(baseph_isEM->at(0));
      if(ph_truthOrigin && baseph_truthOrigin)   ph_truthOrigin->push_back(baseph_truthOrigin->at(0));
      if(ph_truthType && baseph_truthType)       ph_truthType->push_back(baseph_truthType->at(0));
      if(ph_topoetcone40 && baseph_topoetcone40) ph_topoetcone40->push_back(baseph_topoetcone40->at(0));
      if(ph_ptcone20 && baseph_ptcone20)         ph_ptcone20->push_back(baseph_ptcone20->at(0));
      // acceptance cuts
