...
void plotTJVCutTJV(TString outAREA = "/Users/othmanerifki/vbf/TJV/processed"){
...
myCanvas->SaveAs(outAREA+"/plots/TJV/histo_"+histos[i_h]+"_"+process+additional+".pdf");
//myCanvas->SaveAs(outAREA+"/plots/TJV/histo_"+histos[i_h]+"_"+process+additional+".C");
...
for (auto const& x : processratio){
  ...
  myCanvas2->SaveAs(outAREA+"/plots/TJV/eff_"+process+".pdf");
  //myCanvas2->SaveAs(outAREA+"/plots/TJV/eff_"+process+".C");
}
...
for (UInt_t i_h = 0; i_h < N_HIST; i_h++) {
  for (UInt_t i_c = 0; i_c < N_CUT; i_c++) {
    ...
    h_sum[histos[i_h]][cuts[i_c]]->SetMaximum(0.2 * pow(h_sum[histos[i_h]][cuts[i_c]]->GetMaximum() / 0.2, 1.5));
    h_sum[histos[i_h]][cuts[i_c]]->SetFillStyle(3001);
    h_sum[histos[i_h]][cuts[i_c]]->SetFillColor(kGray);
    h_sum[histos[i_h]][cuts[i_c]]->GetXaxis()->SetLabelSize(0.045);
    h_sum[histos[i_h]][cuts[i_c]]->GetXaxis()->SetTitleSize(0.045);
    h_sum[histos[i_h]][cuts[i_c]]->GetXaxis()->SetTitleOffset(1.35);
    h_sum[histos[i_h]][cuts[i_c]]->GetXaxis()->SetLabelOffset(0.03);
  ...
  //h_store[samples[s]][histos[i_h]][cuts[i_c]]->Scale(1, "width");
    ...
    for (int i = low_bin; i <= high_bin; i++) {
      double n_bkg_i = h_sum[histos[i_h]][cuts[i_c]]->GetBinContent(i);
      double n_sig_i = h_store[samples[nBkgSamples]][histos[i_h]][cuts[i_c]]->GetBinContent(i);
  //      std::cout << n_bkg_i << " " << n_sig_i << " " << n_sig_i/sqrt(n_bkg_i) << " " << RooStats::NumberCountingUtils::BinomialExpZ(n_sig_i,n_bkg_i,0.0000001) << " " << n_sig_i/n_bkg_i << std::endl;
    ...
    myCanvas3->SaveAs(outAREA+"/plots/TJV/bkgsig_"+histos[i_h]+"_"+cuts[i_c]+".pdf");
    //myCanvas3->SaveAs(outAREA+"/plots/TJV/bkgsig_"+histos[i_h]+".C");
  }
}
...
myCanvas4->SaveAs(outAREA+"/plots/TJV/SoB_"+histos[i_h]+".pdf");
//myCanvas4->SaveAs(outAREA+"/plots/TJV/SoB_"+histos[i_h]+".C");
TCanvas *myCanvas5 = new TCanvas("myCanvas5", "",0,0,600,500);
TLegend *l5 = new TLegend(0.72,0.67,0.92,0.91);
for (UInt_t i_c = 0; i_c < N_CUT; i_c++) {
  hSsB[i_c]->SetMaximum(35);
...
myCanvas5->SaveAs(outAREA+"/plots/TJV/SsB_"+histos[i_h]+".pdf");
//myCanvas5->SaveAs(outAREA+"/plots/TJV/SsB_"+histos[i_h]+".C");
