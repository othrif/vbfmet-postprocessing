...
parser = argparse.ArgumentParser( description = "Submitting samples to condor for processing with ZHDarkPhAnalysisAlg ", add_help=True , fromfile_prefix_chars='@')

parser.add_argument( "-n", "--nominal", dest = "nominal", action="store_true", default = True, help = "Do nominal only" )
...
parser.add_argument( "-l", "--listSample", type = str, dest = "listSample", default = "/afs/cern.ch/work/s/ssevova/public/dark-photon-atlas/plotting/source/VBFAnalysis/data/v00_mc16a_bkg_CERN_FileMap.p", help = "list of ntuples to run over" )
parser.add_argument( "-f", "--normFile", type = str, dest = "normFile", default = "/afs/cern.ch/work/s/ssevova/public/dark-photon-atlas/plotting/source/VBFAnalysis/data/fout_v00_mc16a_bkg.root", help = "file with the total number of event processed" )
parser.add_argument( "-p", "--proxyName", type = str, dest = "proxyName", default = "/afs/cern.ch/work/s/ssevova/public/dark-photon-atlas/x509up_u39129", help = "proxy file for grid")
...
for syst in systlist:
    ...
    if args.noSubmit:
        break
    runCommand = '''athena VBFAnalysis/ZHDarkPhAnalysisAlgJobOptions.py --filesInput "'''+samplePatternGlobal+'''$1" - --currentVariation '''+syst+''' --normFile '''+args.normFile+UseExtMC
    if isFileMap:
        runCommand+=''' --containerName $2'''
    writeCondorShell(workDir, buildDir, runCommand, syst, "ZHDarkPhAnalysisCondorSub", proxyName=args.proxyName, slc7=args.slc7) #writeCondorShell(subDir, buildDir, syst, runCommand, scriptName="VBFAnalysisCondorSub")
    print listofrunN
    writeCondorSub(workDir, syst, "ZHDarkPhAnalysisCondorSub", listofrunN, listofrunNMC)
